...
class JElementMatchday extends JElement
{
	/**
	 * Element name
	 *
	...
	function fetchElement($name, $value, &$node, $control_name)
	{
		...
		$article->title='';
		if ($value) {
			$query = "SELECT * FROM #__bl_matchday WHERE id = ".$value;
			$db->setQuery($query);
...
//		$html .= "\n &nbsp; <input class=\"inputbox modal-button\" type=\"button\" value=\"".JText::_('Select')."\" />";
		$html .= '<div class="button2-left"><div class="blank"><a class="modal" title="'.JText::_('BLBE_SELMATCHDAY').'"  href="'.$link.'" rel="{handler: \'iframe\', size: {x: 650, y: 375}}">'.JText::_('Select').'</a></div></div>'."\n";
