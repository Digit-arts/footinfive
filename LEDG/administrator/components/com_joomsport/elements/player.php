...
class JElementPlayer extends JElement
{
	...
	function fetchElement($name, $value, &$node, $control_name)
	{
		...
		$article->title = '';
		if ($value) {
			$query = "SELECT * FROM #__bl_players WHERE id= ".$value;
			$db->setQuery($query);
...
//		$html .= "\n &nbsp; <input class=\"inputbox modal-button\" type=\"button\" value=\"".JText::_('Select')."\" />";
		$html .= '<div class="button2-left"><div class="blank"><a class="modal" title="'.JText::_('BLBE_SELPLAYER').'"  href="'.$link.'" rel="{handler: \'iframe\', size: {x: 650, y: 375}}">'.JText::_('BLBE_SELPLAYER').'</a></div></div>'."\n";
