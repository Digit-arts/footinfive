...
class teamJSModel extends JSPRO_Models
{
	...
	function getData()
	{
		...
		if($this->s_id){
			$query = "SELECT md.m_name,m.id as mid,m.m_date,m.m_time, t1.t_name as home, t2.t_name as away, score1,score2,m.is_extra, m.m_played,t1.t_emblem as emb1,t2.t_emblem as emb2, m.betavailable, IF(CONCAT(m.betfinishdate, ' ', m.betfinishtime)>NOW(),1,0) betfinish FROM #__bl_matchday as md, #__bl_match as m, #__bl_teams as t1, #__bl_teams as t2 WHERE m.m_id = md.id AND md.s_id = ".$this->s_id." AND m.published = 1 AND (m.team1_id = ".$this->team_id." OR m.team2_id = ".$this->team_id.") AND m.team1_id = t1.id AND m.team2_id = t2.id ORDER BY m.m_date,m.m_time";
		}else{
			$query = "SELECT DISTINCT(m.id),md.m_name,m.id as mid,m.m_date,m.m_time, t1.t_name as home, t2.t_name as away, m.score1,m.score2,m.is_extra, m.m_played,t1.t_emblem as emb1,t2.t_emblem as emb2, m.betavailable, IF(CONCAT(m.betfinishdate, ' ', m.betfinishtime)>NOW(),1,0) betfinish"
					." FROM #__bl_matchday as md, #__bl_match as m, #__bl_teams as t1, #__bl_teams as t2, #__bl_seasons as s, #__bl_tournament as tr"
					." WHERE ((tr.id=s.t_id AND tr.t_single = '0' AND md.s_id=s.s_id) OR (md.s_id = -1)) AND m.m_id = md.id AND m.published = 1 AND (m.team1_id = ".$this->team_id." OR m.team2_id = ".$this->team_id.") AND m.team1_id = t1.id AND m.team2_id = t2.id AND s.published='1' AND tr.published='1'"
		...
		$query = "SELECT DISTINCT(p.id),p.* FROM #__bl_players as p, #__bl_players_team as t WHERE p.id=t.player_id AND t.team_id = ".$this->team_id." ".($this->s_id?" AND t.season_id=".$this->s_id:"")." AND t.player_join='0' ORDER BY p.first_name,p.last_name";
		$this->db->setQuery($query);
		...
		for($i=0;$i<count($players);$i++){
			...
			$player_table[$i+1][] = $img."<p class='player-name'><a href='".JRoute::_('index.php?option=com_joomsport&amp;task=player&amp;id='.$players[$i]->id.'&amp;sid='.$this->s_id.'&amp;Itemid='.$Itemid)."'>".$this->selectPlayerName($players[$i])."</a></p>";
			if($unbl_matchplayed){
				$query = "SELECT COUNT(*) FROM #__bl_squard as s, #__bl_match as m, #__bl_matchday as md WHERE md.id=m.m_id ".($this->s_id?" AND md.s_id=".$this->s_id:"")." AND m.id=s.match_id AND m.m_played='1' AND s.mainsquard='1' AND (m.team1_id = ".$this->team_id." OR m.team2_id = ".$this->team_id.") AND s.player_id=".$players[$i]->id;
				$this->db->setQuery($query);
				$gamepl = intval($this->db->loadResult());
				$query = "SELECT m.id FROM #__bl_squard as s, #__bl_match as m, #__bl_matchday as md WHERE md.id=m.m_id ".($this->s_id?" AND md.s_id=".$this->s_id:"")." AND m.id=s.match_id AND m.m_played='1' AND s.mainsquard='1' AND (m.team1_id = ".$this->team_id." OR m.team2_id = ".$this->team_id.") AND s.player_id=".$players[$i]->id;
				$this->db->setQuery($query);
				$mids = $this->db->loadResultArray();
				
				$query = "SELECT COUNT(DISTINCT(m.id)) FROM #__bl_subsin as s, #__bl_match as m, #__bl_matchday as md WHERE md.id=m.m_id ".($this->s_id?" AND md.s_id=".$this->s_id:"")." AND m.id=s.match_id AND m.m_played='1' AND (m.team1_id = ".$this->team_id." OR m.team2_id = ".$this->team_id.") AND s.player_in=".$players[$i]->id;
				if(count($mids)){
					$midss = implode(',',$mids);
...
var $s_id = null;
	...
	function _buildDataObject()
	{
		...
		if ($this->get('pages.current') > 1)
		{
			...
			$data->previous->link	= JRoute::_($_SERVER['REQUEST_URI']."?curcal=1&sid=".$this->s_id."&limitstart=".$page);
		}
		...
		if ($this->get('pages.current') < $this->get('pages.total'))
		{
			...
			$data->next->base	= $next;
			$data->next->link	= JRoute::_($_SERVER['REQUEST_URI']."?curcal=1&sid=".$this->s_id."&limitstart=".$next);
			$data->end->base	= $end;
			$data->end->link	= JRoute::_($_SERVER['REQUEST_URI']."?curcal=1&sid=".$this->s_id."&limitstart=".$end);
		}
		...
		for ($i = $this->get('pages.start'); $i <= $stop; $i ++)
		{
			...
			if ($i != $this->get('pages.current') || $this->_viewall)
			{
				...
				$data->pages[$i]->link	= JRoute::_($_SERVER['REQUEST_URI']."?curcal=1&sid=".$this->s_id."&limitstart=".$offset);
			}
		}
		return $data;
	}
