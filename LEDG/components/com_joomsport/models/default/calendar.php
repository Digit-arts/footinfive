...
class calendarJSModel extends JSPRO_Models
{
	...
	function __construct()
	{
		...
		//$this->limitstart	= $this->mainframe->getUserStateFromRequest( 'com_joomsport.callimitstart'.$this->s_id, 'limitstart', 0, 'int' );
	...
	function getData()
	{
		...
		$this->_lists["panel"] = $this->getePanel($this->_lists["teams_season"],@$this->_lists["unable_reg"],$this->s_id,0);
		$this->getCTFilter();
	...
	function getMatchs(){
		...
		//else $date_sql .= " AND m.m_date >= \"".decaler_jour(date("Y-m-d"),-6)."\"";
		if($this->todate){
		...
		if($this->t_single){
			...
			$query = "SELECT m.*,md.m_name,md.id as mdid, t1.t_name as home, t2.t_name as away, t1.id as hm_id,t2.id as aw_id,t1.t_emblem as emb1,t2.t_emblem as emb2,m.betavailable, IF(CONCAT(m.betfinishdate, ' ', m.betfinishtime)>NOW(),1,0) betfinish"
					." , (TO_DAYS(NOW())-TO_DAYS(m.m_date)) as nbre_jours FROM #__bl_matchday as md, #__bl_match as m, #__bl_teams as t1, #__bl_teams as t2"
					." WHERE m.m_id = md.id AND m.published = 1 AND md.s_id=".$this->s_id."  AND m.team1_id = t1.id AND m.team2_id = t2.id "
					.$team_sql
					.$date_sql
	...
	function getTotal()
	{
			if (empty($this->_total))
			{
				...
				else $date_sql .= " AND m.m_date >= \"".decaler_jour(date("Y-m-d"),-8)."\"";
				if($this->todate){
					$date_sql .= " AND m.m_date <= '{$this->todate}'";
				}
