...
class edit_matchJSModel extends JSPRO_Models
{
	...
	function getMPlayers(& $row){
		if($this->t_single){
			...
			$query = "SELECT CONCAT(p.id,'*',s.team_id) as id,CONCAT(p.first_name,' ',p.last_name) as p_name,p.id as pid"
					...
					." WHERE s.player_id = p.id AND s.team_id = ".$row->team1_id." AND s.season_id=".$this->season_id
			...
			$query = "SELECT CONCAT(p.id,'*',s.team_id) as id,CONCAT(p.first_name,' ',p.last_name) as p_name,p.id as pid"
					." FROM #__bl_players as p, #__bl_players_team as s"
					." WHERE s.player_id = p.id AND s.team_id = ".$row->team2_id." AND s.season_id=".$this->season_id
					." ORDER BY p.first_name,p.last_name";
	...
	function saveAdmmatch(){
		...
		$row->m_time = JRequest::getVar( 'd_time', '', 'post', 'string' );
		...
		$query = "SELECT s.s_id as id, CONCAT(t.name,' ',s.s_name) as name,t.t_type,t.t_single FROM #__bl_tournament as t, #__bl_seasons as s WHERE s.s_id = ".($season_id)." AND s.t_id = t.id";
		$this->db->setQuery($query);
