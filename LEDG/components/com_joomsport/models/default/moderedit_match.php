...
class moderedit_matchJSModel extends JSPRO_Models
{
	var $_data = null;
	var $_lists = null;
	var $season_id = null;
	...
	function getPlEvent($id){
		...
		$query = "SELECT me.*,ev.e_name,CONCAT(p.first_name,' ',p.last_name) as p_name"
				...
				." ORDER BY CAST(me.minutes AS UNSIGNED),p.first_name,p.last_name";
		$this->db->setQuery($query);
	...
	function getMPlayers(& $row){
		if($this->t_single){
			...
			$query = "SELECT CONCAT(p.id,'*',s.team_id) as id,CONCAT(p.first_name,' ',p.last_name) as p_name,p.id as pid"
					...
					." WHERE s.player_id = p.id AND s.team_id = ".$row->team1_id." AND s.player_join='0' AND s.season_id=".$this->season_id
			...
			$query = "SELECT CONCAT(p.id,'*',s.team_id) as id,CONCAT(p.first_name,' ',p.last_name) as p_name,p.id as pid"
					." FROM #__bl_players as p, #__bl_players_team as s"
					." WHERE s.player_id = p.id AND s.team_id = ".$row->team2_id." AND s.player_join='0' AND s.season_id=".$this->season_id
					." ORDER BY p.first_name,p.last_name";
			if($this->season_id == -1){
	...
	function saveModerMatch(){
		...
		$tourn = $this->getTournOpt($season_id);
		...
		if(isset($_POST['extraf']) && count($_POST['extraf'])){
			foreach($_POST['extraf'] as $p=>$dummy){
				echo $query = "DELETE FROM #__bl_extra_values WHERE f_id = ".$_POST['extra_id'][$p]." AND uid = ".$row->id;
				$this->db->setQuery($query);
				$this->db->query();
	...
	function inviteModerMatch(){
		...
		if($is_minv){
			 ...
			 $query = "SELECT u.email,p.id FROM #__bl_players as p, #__bl_players_team as t, #__users as u WHERE u.id = p.usr_id AND p.id=t.player_id AND t.team_id = ".$this->tid." AND t.season_id=".$this->s_id." ORDER BY p.first_name,p.last_name";
		}
		...
		if(count($pl_inv)){
			foreach($pl_inv as $pl){
				...
				if($pl->email){
					...
					$return = JUtility::sendMail($mailfrom, $fromname, $pl->email, $inv_mtitle, $inv_mtext, 1);
