...
class invitingJSModel extends JSPRO_Models
{
	...
	function getData()
	{
		$messaga = JText::_('BLFA_MESS_STWRONG');
		//title
		...
		if($key != ''){
			...
			if($this->db->loadResult()){
				...
				$this->db->setQuery($query);
				$this->db->query();
				$messaga = JText::_('BLFA_YOUINTEAM');
			}
	...
	function JoinTeam(){
		...
		if($team_id && $s_id && $this->getJS_Config('esport_join_team')){
			...
			$this->db->setQuery($query);
			$this->db->query();	
			$messaga = JText::_("BLFA_WAIT_FOR_APPROVAL");
