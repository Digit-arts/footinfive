...
<?php
		...
		<table class="season-list" cellpadding="0" cellspacing="0" border="0">
			...
			<tr>
				...
				<td>
					...
					if($lists['s_enbl_extra'] && $lists['t_type']){
						echo '&nbsp;'.JText::_("AET").'<input type="text" name="aet1" value="'.$row->aet1.'" size="5" maxlength="5" />&nbsp;'.JText::_("W").'<input type="checkbox" id="spenwin_1" '.(( $row->p_winner && $row->p_winner == $row->team1_id)?"checked":"").' name="penwin[]" value="'.$row->team1_id.'" onchange="sqchng(\'spenwin_1\',\'spenwin_2\');" />';
					}
					echo '&nbsp;:&nbsp;'.$lists['teams2'].'&nbsp;<input type="text" name="score2" value="'.$row->score2.'" size="5" maxlength="5" />';
					if($lists['s_enbl_extra'] && $lists['t_type']){
						echo '&nbsp'.JText::_("AET").'<input type="text" name="aet2" value="'.$row->aet2.'" size="5" maxlength="5" />&nbsp;'.JText::_("W").'<input type="checkbox" id="spenwin_2" '.(( $row->p_winner && $row->p_winner == $row->team2_id)?"checked":"").' onchange="sqchng(\'spenwin_2\',\'spenwin_1\');" name="penwin[]" value="'.$row->team2_id.'" />';
					}
					?>
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					<input type="text" maxlength="5" class="feed-back inp-small" size="10" name="d_time" value="<?php echo substr($row->m_time,0,5);?>" />
...
<tr>
	...
	<div style="margin-top:10px;border:1px solid #BBB;">
		...
		<table class="adminlist">
			...
			?>
			<td align="center">
				<a href="javascript:void(0);" title="Remove" onClick="javascript:Delete_tbl_row(this);"><img src="<?php echo JURI::base();?>components/com_joomsport/img/ico/close.png" title="Remove" /></a>
