...
if ($user_group==3 or $user_group==8 ){
	...
	if (test_non_vide($_POST["nbre_vainq_par_poule"])){
		...
		else echo "<br>Le produit (Nombre de poule)X(Nombre de vainqueur par poule) = "
                            .($_POST["nbre_vainq_par_poule"]*$nbre_poules)." ne s'ecrit pas sous la forme d'une puissance de 2.<br><br>";
	}
		...
		while ($nbre_equipes>=2){
			for ($e=1;$e<=$nbre_equipes;$e++){
				...
				if (($e%2)==0){
					...
					else $tab_infos_rencontres[$nbre_equipes][($e-1)]["equipe2"]=0;
					...
					if (!test_non_vide($_POST["$l_heure"]) or !test_non_vide($_POST["$la_date"]) or !test_non_vide($_POST["$le_terrain"])){
						...
						if (test_non_vide($erreur))
							break;
						else $tab_cles_date_heure_terrain[]=$la_cle_actuelle;
					}
				}
				else {
					if ($nbre_equipes==$nbre_equipes_init){
					...
					else $tab_infos_rencontres[$nbre_equipes][$e]["equipe1"]=0;
				}
			}
			$nbre_equipes/=2;
		...
		if (test_non_vide($erreur))
			...
			while ($nbre_equipes<=$nbre_equipes_init){
				for ($i=1;$i<=($nbre_equipes-1);$i=($i+2)){
					if (($nbre_equipes-1)==1)						
						$k_title="Finale";
					else $k_title="1/".($nbre_equipes/2);
						
					ajout_match($mday_differentes_phase_finale,$tab_infos_rencontres[$nbre_equipes][$i]["equipe1"],
						    $tab_infos_rencontres[$nbre_equipes][$i]["equipe2"],
						    $tab_infos_rencontres[$nbre_equipes][$i]["date"],
						    $tab_infos_rencontres[$nbre_equipes][$i]["heure"],
						    $tab_infos_rencontres[$nbre_equipes][$i]["terrain"],$k_ordering,$k_title,$k_stage);
    ...
    if ((isset($_POST["liste_tourn"]) and $_POST["liste_tourn"]==0)
        or (isset($_POST["nom_tournoi"]) and $_POST["nom_tournoi"]!="")){
        if (isset($_POST["nom_tournoi"]) and $_POST["nom_tournoi"]!=""){
            ...
            if (isset($_POST["Champ"])){
                ...
                if ($_POST["Coupe"]==1 or ($puissance_de_2>0 and ($nbre_equipes_phase_finale==$_POST["format_coupe"]))){
                    ...
                    if ($puissance_de_2==0)
                        echo "<br>Le produit (Nombre de poule)X(Nombre de vainqueur par poule) = "
                            ."".($_POST["nbre_vainq_par_poule"]*$_POST["nbre_poules"])." ne s'ecrit pas sous la forme d'une puissance de 2.<br><br>";
                    if ($nbre_equipes_phase_finale!=$_POST["format_coupe"])
                        echo "Le format de la coupe (".$_POST["format_coupe"]." &eacute;quipes) ne correspond pas au nombre d'&eacute;quipes "
                            ." en phase finale (".$nbre_equipes_phase_finale." &eacute;quipes).<br><br>";
            ...
            echo "</select>";
            echo "<H3>Coupe</H3>";
            echo "Type de coupe <select name=\"Coupe\">"
                    ."<option value=\"3\">Coupe avec poules</option>"
                    ."<option value=\"2\">Coupe sans poules</option>"
                    ."<option value=\"1\">Sans coupe</option></select><br>";
            echo "Format de la coupe <select name=\"format_coupe\">"
                    ."<option value=\"4\">1/2</option>"
                    ."<option value=\"8\">1/4</option>"
                    ."<option value=\"16\">1/8</option>"
                    ."<option value=\"32\">1/16</option>"
                    ."<option value=\"64\">1/32</option></select><br>";
	...
	foreach (liste_tourn("",""," nom_tourn ") as $liste_tourn) {
		...
		echo "<option value=\"".$liste_tourn->id_tourn."_".$liste_tourn->id_saison."_".$liste_tourn->gid."_"
		    .$liste_tourn->nbre_equipes."_".$le_nbre_equipes_par_groupe."_".$liste_tourn->nbre_equipes_total."\" ";
		...
		echo ">".$liste_tourn->nom_tourn." (".$liste_tourn->nom_saison." - ".$liste_tourn->group_name." - "
		    .$liste_tourn->nbre_equipes."/".$le_nbre_equipes_par_groupe." equipes)</option>";
	...
	foreach (liste_tourn(" and t.t_type=1 ",1) as $liste_tourn) {
		...
		echo ">".$liste_tourn->nom_tourn." (".$liste_tourn->nom_saison." - "
		    .$le_nbre_equipes_par_groupe." equipes)</option>";
        }
        echo "</select>";
	if (test_non_vide($id_tourn))
			echo " <a title=\"Supprimer ce tournoi\" onclick=\"recharger('Confirmez la suppression de ce tournoi','gen?suppr_tourn=$id_tourn')\">"
		...
		echo "<br><br>Selectionnez la date de debut du tournoi ainsi que la date de reprise pour les matchs retours.<br><br>"
			."Date de debut : <input type=\"date\" name=\"date_debut\" value=\"".$date_debut."\">";
		if (isset($_POST["heure_debut"])) $heure_debut_complete = $_POST["heure_debut"];
				else $heure_debut_complete = $_GET["heure_debut"];
			list($heure_debut,$minutes_debut) = explode(':', $heure_debut_complete );
		echo "<select name=\"heure_debut\">";
		for ($i=9;$i<=21;$i++){
			if (($heure_debut==$i)) $select_heure=" selected ";
			else $select_heure="";
		...
		echo "Type de matchs "
		        ." <select name=\"aller_retour\"><option value=\"1\" >allers/retours</option>"
		        ." <option value=\"2\" >allers</option></select><br>";
        }
    ...
    if (isset($id_saison)){
        if ($nbre_equipes_manquantes>0){
            ...
            if (isset($_POST["Groupe"])){
                ...
                else echo "<br><font color=red>Nombre d'equipes selectionn&eacute;es (".$nbre_equipes_cochees.") est superieur au nombre d equipes manquantes (".$nbre_equipes_manquantes.").</font><br>";
            }
            else {
                ...
                if (test_non_vide($resultat_groups_sans_equipes) and ($nbre_equipes_manquantes>0)){
                    ...
                    echo "<br><br><u>Selectionner les ".$nbre_equipes_manquantes
			." equipes qui joueront en <b><font color=red>".$resultat_groups_sans_equipes->group_name."</font> :</u></b><br>";
                    $min=0;$max=0;
		...
		//echo "<br>heure deb:".$heure_debut_seule." - heure fin ".$heure_fin_seule." - decal minutes ".(ceil($nbre_equipes_par_tourn/2/$nbre_terrains_fif)*60);
                                        ...
                                        for ($Equipe_B=1;$Equipe_B<=$nbre_equipes;$Equipe_B++){
                                                if ($tab_journee[$Equipe_A][$Equipe_B]<>"" and !in_array($liste_groupe->id_du_groupe."_".$num_match, $tab_num_match_deja_attribuer)){
                                                        ...
                                                        if ($terrain<>"" and $heure<>""){
                                                            $tab_matchs[]=array("journee" => $id_journee_match,
                                                                                "heure" => $heure.":00",
                                                                                "terrain" => "T".$terrain,
                                                            ...
                                                            if ($_POST["aller_retour"]==1)
                                                                $tab_matchs[]=array("journee" => ($id_journee_match+$nbre_equipes-1),
                                                                                "heure" => $heure.":00",
                                                                                "terrain" => "T".$terrain,
                                                                                "poule" => $liste_groupe->id_du_groupe,
                                                                                "num_match" => $num_match,
			...
			$tab_cles_date_heure_terrain = array();
                        ...
                        for ($i=1;$i<=($nbre_equipes-1);$i++){
                            ...
                            echo "<table width=\"100%\"><tr><td colspan=\"4\" nowrap>-------------ALLER----- Journ&eacute;e ".$i
				." : <input type=\"date\" name=\"date_journee_".$i."\" ".$readonly." value=\"";
				...
				if ($match["journee"]==$i ){
				    ...
				    else $old_terrain=$match["terrain"];
				    ...
				    echo "<tr><td nowrap>".nom_equipe($match["id_equipe_A"])."</td><td nowrap>".nom_equipe($match["id_equipe_B"])
					."</td><td>".$le_select_heure."</td><td>".$le_select_terrain."</td><td width=\"100%\">".$erreur."</td></tr>";
				...
				foreach ($tab_matchs as $match)
					...
					echo "<tr><td width=\"100%\" align=\"right\">".$erreur."</td><td nowrap>".nom_equipe($match["id_equipe_A"])."</td><td nowrap>"
						.nom_equipe($match["id_equipe_B"])."</td><td>".$le_select_heure."</td><td><b>".$le_select_terrain."</td></tr>";
			...
			if (!(test_non_vide($_POST["valider_dates"]) and $_POST["valider_dates"]==1) and $nbre_creneaux_pb_dispo==0)
                            echo "<input type=\"checkbox\" name=\"valider_dates\" value=\"1\">Cochez pour valider les dates et generer les matchs correspondants"
				."<br><font color=red><b>Ne jamais cocher</b> sans avoir au pr&eacute;alable generer les matchs et verifier qu'aucune indisponilit&eacute; n'est signal&eacute;e !</font>";
                        else $texte_bouton="Verifier de nouveau les dispos";
			...
			if ($nbre_creneaux_pb_dispo<>0)
				echo "<font color=yellow size=4>Pb dispo dates sur FIF sur ".$nbre_creneaux_pb_dispo." rencontres</font>";
                    ...
                    else echo "<br><br>Erreur : Le nombre d'equipes est impair<br><br>";
            }
        }
    }    
	...
	if (test_non_vide($_POST["liste_tourn"]) and (substr(recup_1_element("group_name","#__bl_groups","id",$id_groupe),0,5)=="Poule")
	    and existe_tourn(substr(recup_1_element("name","#__bl_tournament","id",$id_tourn),0,-21)." - Phase finale Coupe")==0
	    and !(test_non_vide($resultat_groups_sans_equipes) and ($nbre_equipes_manquantes>0))
	    and !(isset($id_saison) and ($nbre_equipes_inscrites_a_la_saison==$nbre_equipes_par_tourn) and (journees_generes($id_saison)==0))){
	    ...
	    while ($nbre_equipes>=2){
		    ...
		    for ($e=1;$e<=$nbre_equipes;$e++){
			    ...
			    if (($e%2)==0){
				    ...
				    echo "Date du match <input type=\"date\" name=\"".$la_date."\" onChange=\"valider()\" value=\"".$_POST["$la_date"]."\">";
				    echo menu_deroulant_des_heures_avec_dispo_fif($l_heure,$_POST["$l_heure"],$_POST["$la_date"]," onChange=\"valider()\" ");
				    
				    echo menu_deroulant_des_terrains_avec_dispo_fif($le_terrain,$_POST["$le_terrain"],$_POST["$la_date"],$_POST["$l_heure"]," onChange=\"valider()\" ");
    ...
    else echo "Creation des matchs de la phase finale termin&eacute;e !<br>Reste plus qu'&agrave les valider dans FIF en allant dans TODAY !";
