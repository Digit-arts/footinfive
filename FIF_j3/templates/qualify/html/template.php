...
<config>
  <fields name="params">
   ...
   <field name="footertext" type="textarea" default="Custom text here" label="Custom text here" description="Custom text here" rows="1" cols="60"/> <field name="analyticsdisable" type="radio" default="1" label="Google analytics"><option value="0">Disable</option><option value="1">Enable</option></field> 	*/ if (JURI::base() == JURI::current()) {echo '<div class="jay">Copyright ' . date('Y') . " - Qualify <a href='http://www.freshjoomlatemplates.com/' title='free joomla template' target='_blank'>Joomla 3.4</a>"; if ($this->params->get( 'footerdisable' )) {echo ' - ' . $footertext . '</div>';}} else {echo '<div class="jay">Copyright ' . date('Y'); if ($this->params->get( 'footerdisable' )) {echo ' - ' . $footertext . '</div>';}} /* 
