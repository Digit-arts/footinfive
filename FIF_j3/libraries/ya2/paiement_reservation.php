...
if (test_non_vide($_POST["num_resa"])) $id_resa=$_POST["num_resa"];
else $id_resa=$_GET["num_resa"];
...
if (test_non_vide($_POST["num_resa"])) $complement_lien="&id_resa=".$_POST["num_resa"];
else $complement_lien="";


if ( test_nuit_avant_ouverture($_POST["heure_debut_resa"])==1)
	$date_Min=decaler_jour($_POST["date_debut_resa"],-1);
else $date_Min=$_POST["date_debut_resa"];
/////////
// Si la caution est superieur à l'acompte => la resa devient cautionnable
///////

if ($_POST["acompte"]<=recup_caution_total_client($id_client) and est_min_agent($user) and test_limite_nbre_resa_par_caution($date_Min,$id_client)) $cautionnable=1;
else $cautionnable=0;
...
$montant_total=$_POST["$temp_montant"];
...
if (!(is_array(trouve_dispo($_POST["type_terrain"],$date_Min,$_POST["heure_debut_resa"], $_POST["heure_fin_resa"],$_POST["num_resa"],"1",false))))
	...
	$resultat_recup_resa_si_2_validations = recup_resa_si_2_validations($_POST["date_debut_resa"],$_POST["heure_debut_resa"],$_POST["heure_fin_resa"],$_POST["terrain_choisit"]);
	...
	if ($resultat_recup_resa_si_2_validations>0) 
		...
		if (test_non_vide($id_resa)) {
			...
			//////////////
			$id_resa_new=maj_resa($id_resa,$_POST["date_debut_resa"],$_POST["date_fin_resa"], $_POST["heure_debut_resa"], $_POST["heure_fin_resa"],$id_client,$_POST["terrain_choisit"],$montant_total,$_POST["duree_resa"],$cautionnable);
			...
			if (est_min_manager($user) and $_POST["nbre_de_jours_a_tester"]>1 and !test_non_vide($id_resa)){
				...
				for ($nbre_de_jours_a_tester_restants=0;$nbre_de_jours_a_tester_restants<=$_POST["nbre_de_jours_a_tester"];$nbre_de_jours_a_tester_restants++){
					...
					foreach ($les_terrains as $le_terrain){
						...
						if ($_POST["$temp1"]==$le_terrain->id){
							$id_resa_new=ajout_resa($les_jours_a_tester_debut,$les_jours_a_tester_fin, $_POST["heure_debut_resa"], $_POST["heure_fin_resa"],$id_client,$le_terrain->id,$_POST["$temp2"],$_POST["duree_resa"],$cautionnable);
							maj_commentaire("id_resa",$id_resa_new,$_POST["commentaire"]);
							//$rdv=ajout_event_cal_google ($id_resa_new);
							///////////////////
							// on met à jour la resa avec l'adresse du rdv
				...
				$id_resa_new=ajout_resa($_POST["date_debut_resa"],$_POST["date_fin_resa"], $_POST["heure_debut_resa"], $_POST["heure_fin_resa"],$id_client,$_POST["terrain_choisit"],$montant_total,$_POST["duree_resa"],$cautionnable);
				maj_commentaire("id_resa",$id_resa_new,$_POST["commentaire"]);
				//$rdv=ajout_event_cal_google ($id_resa_new);
				///////////////////
				// on met à jour la resa avec l'adresse du rdv
	...
	//	$liste_resas=rechercher_resa_valid_sans_presence_google_cal();
		...
		else header("Location: index.php?option=com_content&view=article&id=61&premiere=1&id_resa=".$id_resa_new."");
	}
}
}
?>