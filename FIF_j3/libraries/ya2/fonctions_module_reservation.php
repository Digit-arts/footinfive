...
function maj_connect($user, $IP) {
	...
	if (! test_non_vide ( $derniere_ip ) or (test_non_vide ( $derniere_ip ) and $IP != $derniere_ip)) {
		$requete_maj_connect = "INSERT INTO `Connect`(`id_user`, `date`, `heure`, `IP`) " . " VALUES (" . $user->id . ",\"" . date ( "Y-m-d" ) . "\",\"" . date ( "H:i" ) . "\",\"" . $IP . "\") ";
...
function maj_commentaire($champs, $valeur_champs, $commentaire) {
	...
	if ((! test_non_vide ( $dernier_commentaire->Commentaire ) and test_non_vide ( $commentaire )) or (test_non_vide ( $dernier_commentaire->Commentaire ) and $dernier_commentaire->Commentaire != $commentaire)) {
		$requete_insert_commentaire = "INSERT INTO `Commentaires`(`id_user`, `date`, `heure`, " . $champs . ", `Commentaire`) " . " VALUES (" . $user->id . ",\"" . date ( "Y-m-d" ) . "\",\"" . date ( "H:i" ) . "\"," . $valeur_champs . ",\"" . $commentaire . "\")";
...
function recup_rencontre($id_match) {
	...
	$query_recup_rencontre = "SELECT CONCAT( `k_title` ,\"<br>\", (SELECT t.t_name FROM vlxhj_bl_teams AS t WHERE  `team1_id` = t.id),\": \", " . " (IF(`score1`>0,`score1`,\"-\")) ,\"<br>\",(SELECT t.t_name FROM vlxhj_bl_teams AS t WHERE  `team2_id` = t.id ),\": \",(IF(`score2` >0,score2,\"-\")) )" . "   as rencontre FROM  `vlxhj_bl_match`  WHERE  `id`=" . $id_match;
...
function photo_user($id_user) {
	...
	$query = "select ph.ph_filename from vlxhj_bl_photos as ph where ph.id=(if(" . $def_img [0] . ">0," . $def_img [0] . ",(select max(photo_id) " . " from vlxhj_bl_assign_photos as ass_ph where ass_ph.cat_type = 1 AND cat_id =" . $id_user . "))) ";
	// echo $query;
...
function existe_joueur_capitaine($id_user) {
	...
	$requete_existe_joueur = "SELECT u.id, t_name FROM  `vlxhj_users` AS u LEFT JOIN vlxhj_bl_moders ON uid = u.id " . " LEFT JOIN vlxhj_bl_teams AS t ON tid = t.id WHERE u.`id` =" . $id_user;
...
function ajout_user_au_groupe($user_id, $groupe) {
	...
	$requete_insert_user_group = "INSERT INTO #__user_usergroup_map (`user_id`, `group_id`)" . "VALUES (" . $user_id . "," . $groupe . ");";
...
function entite_du_client($id_client) {
	...
	$requete_nom_du_client = "Select concat(\"<b>\",(select tr.nom FROM Type_Regroupement as tr WHERE tr.id=id_type_regroupement),\"</b> \",nom_entite,\"<br>\",nom_service) " . " from Client where id_client=" . $id_client;
	$db->setQuery ( $requete_nom_du_client );
	return ($db->LoadResult ());
}
...
function recup_dest_publipostage($id_pub) {
	...
	foreach ( $resultat as $elt ) {
		switch ($elt->id_type_regroupement) {
			case 10000 :
				$liste_destinatires .= "POLICE, ";
				break;
			case 10001 :
				$liste_destinatires .= "LEDG, ";
				break;
			case 0 :
				$liste_destinatires .= "Particulier, ";
				break;
			case ($elt->id_type_regroupement < 10000) :
				$liste_destinatires .= recup_1_element ( "nom", "Type_Regroupement", "id", $elt->id_type_regroupement ) . ", ";
				break;
			default :
				$liste_destinatires .= "pb requete(" . $elt->id_type_regroupement . "),";
				break;
		}
	}
	...
	if (test_non_vide ( $cp ) and ! (VerifierCP ( $cp ))) {
		...
		if ($resultat_verif_si_cp_existe == 0 or $resultat_verif_si_cp_existe == "") {
			...
			if (! (test_non_vide ( $cp )) and (test_non_vide ( $ville ))) {
				...
				if ($nbre_cp == 1)
					$code_insee = $tab_cp [0] ["INSEE"];
				else if ($nbre_cp == 0)
					echo "<font color=red>Ville inconnue.<br></font>";
				$existe_erreur ++;
			}
		}
	}
	...
	if (test_non_vide ( $ville ) and ! (VerifierNomVille ( $ville ))) {
		...
		if ((test_non_vide ( $cp )) and (test_non_vide ( $ville ))) {
			...
			if ($resultat_verif_si_ville_existe == "") {
				...
				$code_insee = $resultat_verif_si_ville_existe;
		}
	}
}
				...
				case 0 :
					{
						if (isset ( $modif ))
							...
							echo "<tr><th>Code postal :	</th><td>" . $cp . "</td></tr> ";
						break;
					}
				...
				case 1 :
					{
						if (isset ( $modif ))
							...
							echo "<tr><th>Code postal :	</th><td>" . $tab_cp [0] ["CP"] . "</td></tr> ";
						break;
					}
				...
				case 1 :
					{
						if (isset ( $modif ))
							...
							echo "<tr><th>Ville :	</th><td>" . $tab_villes [0] ["VILLE"] . "</td></tr>";
						break;
					}
...
function ajout_presta($id_resa, $type_presta, $montant, $tva) {
	...
	$requete_ajout_presta = "INSERT INTO `Prestation`(`type_prestation`, `id_TVA`, `Montant_TTC`,id_resa,`id_user`, `date_creation`, `heure_creation`) " . " VALUES (\"" . $type_presta . "\"," . $tva . "," . str_replace ( ",", ".", $montant ) . "," . $id_resa . "," . $user->id . ",\"" . date ( "Y-m-d" ) . "\",\"" . date ( "H:i" ) . "\") ";
...
function ajout_doc($id_client, $nom_doc, $nom_fichier) {
	...
	$requete_ajout_doc = "INSERT INTO `Document`(`nom_doc`, `nom_fichier`, `id_client`, `id_user`, `date_ajout`, `heure_ajout`) " . " VALUES (\"" . $nom_doc . "\",\"" . $nom_fichier . "\"," . $id_client . "," . $user->id . ",\"" . date ( "Y-m-d" ) . "\",\"" . date ( "H:i" ) . "\") ";
...
function nbre_contremarques_utlisees($id_tarif, $date_limite, $date_crea, $heure_crea, $id_client) {
	...
	$requete_nbre_clef_deja_utilisee = "SELECT `id_reservation` FROM `Reglement` WHERE `Clef` in (SELECT Clef FROM `Contremarque` " . " WHERE `id_tarif`=\"" . $id_tarif . "\" and `date_limite`=\"" . $date_limite . "\"  and `date_crea`=\"" . $date_crea . "\" " . " and `heure_crea`=\"" . $heure_crea . "\" and `id_client`=\"" . $id_client . "\" ) ";
...
function recup_id_tarif_d_une_resa($id_resa) {
	...
	$requete_recup_id_tarif_d_une_resa = "SELECT tp.id_tarif FROM `Reservation` as r, Tarif_periode as tp, Tarif_periode_type_terrain as tptt, Terrain as t " . " WHERE r.`id_resa`=" . $id_resa . " and r.id_terrain=t.id " . " and r.tarif_horaire=tptt.`montant_horaire` and tptt.id_periode=tp.id_periode and tptt.id_type_terrain=t.id_type " . " and date_debut_resa<=tp.`date_fin_periode` and date_debut_resa>=tp.`date_debut_periode` ";
...
function recup_taux_TVA_d_une_resa($id_resa) {
	...
	$requete_recup_taux_TVA_d_une_resa = "SELECT tp.taux_TVA FROM `Reservation` as r, Tarif_periode as tp, Tarif_periode_type_terrain as tptt, Terrain as t " . " WHERE r.`id_resa`=" . $id_resa . " and r.id_terrain=t.id " . " and r.tarif_horaire=tptt.`montant_horaire` and tptt.id_periode=tp.id_periode and tptt.id_type_terrain=t.id_type " . " and date_debut_resa<=tp.`date_fin_periode` and date_debut_resa>=tp.`date_debut_periode` LIMIT 0,1 ";
...
function recup_taux_TVA_d_une_date($date) {
	...
	$requete_recup_taux_TVA_d_une_date = "SELECT taux_TVA FROM Tarif_periode WHERE " . "  \"" . $date . "\"<=`date_fin_periode` and \"" . $date . "\">=`date_debut_periode` LIMIT 0,1 ";
...
function maj_resa_a_supprimer($id_resa) {
	...
	$requete_maj_resa_a_supprimer = "UPDATE Reservation as r SET a_supprimer=1 where (r.indic_annul<>1) and r.id_resa=" . $id_resa . " and " . " ((SELECT c.`accompte_necessaire` FROM `Client` as c WHERE c.`id_client`=r.`id_client`)=0)  and " . " ( (select count(`id_reservation`) from Reglement where validation_reglement=1 " . " and id_reservation=" . $id_resa . " and (id_remise is NULL or id_remise=0) )=0 " . " and (TIMESTAMPDIFF(MINUTE,CAST(concat(r.date_valid,\" \",r.heure_valid) AS CHAR(22)),NOW())>5) " . " and r.accompte_necessaire=0 and r.cautionnable=0)" . " OR (r.indic_venue=3 and r.cautionnable in (1,2)) " . " OR ((TIMESTAMPDIFF(MINUTE,CAST(concat(r.date_suppr_caution,\" \",r.heure_suppr_caution) AS CHAR(22)),NOW())>1440) " . " AND r.cautionnable=2) ";
...
function versements_sans_remise_et_avec_validation($id_resa, $exclure_moyens_paiement = "") {
	...
	$requete_versement = "select format(sum(reg.montant_reglement),2) as total_versement from Reglement as reg " . "  where reg.validation_reglement=1 " . $compl_req . " and reg.id_reservation=" . $id_resa . " and (reg.id_remise is null or reg.id_remise=0)";
...
function proprietaire_resa($id_resa) {
	...
	if (est_register ( $user ) and test_non_vide ( $id_resa )) {
		...
		if ($nbre_resas == 0) {
			echo "Vous n &ecirc;tes pas propri&eacute;taire de cette r&eacute;servation.";
			exit ();
		} else
			return (true);
	}
...
function recup_caution_total_client($id_client) {
	...
	$requete_recup_montant_caution_total = "SELECT IFNULL((SELECT sum(credit) FROM Credit_client where type_credit=2 and validation_credit=1 and id_client=" . $id_client . "),0) ";
...
function recup_credit_total_client($id_client) {
	...
	$requete_recup_montant_credit_total = "SELECT IFNULL((SELECT sum(credit) FROM Credit_client where type_credit=1 and validation_credit=1 and id_client=" . $id_client . "),0) ";
...
function ajout_credit($id_client, $montant, $type_credit, $origine_credit, $id_moyen_paiement, $validation = 1, $info_credit = "") {
	...
	$requete_trouve_credit = "select id_credit_client from `Credit_client` " . " WHERE id_client=" . $id_client . " AND id_user_creation=" . $user->id . " AND type_credit=\"" . $type_credit . "\"" . " AND credit=" . str_replace ( ",", ".", $montant ) . " AND date_credit=\"" . date ( "Y-m-d" ) . "\"" . " AND heure_credit=\"" . date ( "H:i" ) . "\"" . " AND origine_credit=\"" . $origine_credit . "\"" . " AND id_moyen_paiement=" . $id_moyen_paiement . " AND validation_credit=" . $validation . " AND info_credit=\"" . $info_credit . "\"";
...
function menu_deroulant_des_users($etat, $old_select) {
	...
	$requete_recup_liste = "Select u.* from #__users as u, #__user_usergroup_map as ugm where ugm.user_id=u.id and ugm.group_id>2 and ugm.group_id<8 " . " and block=" . $etat . " order by username asc";
	// echo "<br><br>".$requete_recup_liste;
	$db->setQuery ( $requete_recup_liste );
	$resultat_recup_liste = $db->loadObjectList ();
...
function liste_des_champs($Table, $deb_fin, $test_erreurs, $old_saisie = "", $recup_id_journee = "", $test_si_saisie_qqchose = "", $modifier = "") {
	...
	foreach ( $resultat_recup_liste as $recup_liste ) {
		...
		if (test_non_vide ( $resultat_decompo_deb->nbre ) and ! test_non_vide ( $test_si_saisie_qqchose ["$deb_fin"] ))
			...
			echo $old_saisie ["$deb_fin"] [$recup_liste->id];
		echo "\"></td><td align=\"right\">";
		if (test_non_vide ( $resultat_decompo_deb->nbre ) and ! test_non_vide ( $test_si_saisie_qqchose ["$deb_fin"] ))
			echo ($recup_liste->valeur * $resultat_decompo_deb->nbre);
		else
			echo ($recup_liste->valeur * $old_saisie ["$deb_fin"] [$recup_liste->id]);
		echo " &#8364;</td></tr>";
	}
}
...
function requete_resas_a_supprimer($id_resa = "") {
	if (test_non_vide ( $id_resa ))
		$comp_req = " and  r.id_resa=" . $id_resa;
	else
		$comp_req = "";
...
function nettoyer_resa_non_payees() {
	...
	if (test_non_vide ( $_GET ["venue"] )) {
		...
		if ($resultat_recup_venue_actuelle < 3)
			$new_etat_venue = $resultat_recup_venue_actuelle + 1;
		else
			$new_etat_venue = 1;
	...
	if ($nbre_resultats > 0) {
		...
		foreach ( $resultat_recup_resa as $recup_resa ) {
			if ($recup_resa->indic_venue == 3 and $recup_resa->cautionnable == 1) {
				...
				annuler_resa ( $recup_resa->id_resa, 7 );
		}
	}
}
...
function test_valid_existence_date($date) {
	...
	if (($annee != "") and ($mois != "") and ($jour != "") and checkdate ( $mois, $jour, $annee ))
		return (true);
	else
		return (false);
}
...
function test_nuit_avant_ouverture($horaire) {
	if (diff_dates_en_minutes ( "1970-01-01", $horaire, "1970-01-01", horaire_ouverture () ) <= 0)
		return (0);
	else
		return (1);
}
...
function test_horaires_ouverture($horaire1, $horaire2) {
	if ($horaire1 == "")
		$horaire1 = horaire_ouverture ();
	if ($horaire2 == "")
		$horaire2 = horaire_fermeture ();
	...
	if ((($heure1 + 0) < ($heure_ouverture + 0)) and (($heure2 + 0) < ($heure_ouverture + 0)))
		$date1 = "1970-01-02";
	else
		$date1 = "1970-01-01";
	
	if (($heure2 + 0) < ($heure_ouverture + 0))
		$date2 = "1970-01-02";
	else
		$date2 = "1970-01-01";
	
	if (($heure_fermeture + 0) < ($heure_ouverture + 0))
		$date_fermeture = "1970-01-02";
	else
		$date_fermeture = "1970-01-01";
	...
	if (diff_dates_en_minutes ( $date1, $horaire1, "1970-01-01", horaire_ouverture () ) > 0)
		return (0);
	if (diff_dates_en_minutes ( $date2, $horaire2, $date_fermeture, horaire_fermeture () ) < 0)
		return (0);
...
function diff_dates_en_minutes($date1 = "", $horaire1 = "", $date2 = "", $horaire2 = "") {
	...
	if ($date1 == "")
		$date1 = date ( "Y-m-d" );
	if ($horaire1 == "")
		$horaire1 = date ( "H" ) . ":" . date ( "i" );
	if ($horaire2 == "")
		$horaire2 = date ( "H" ) . ":" . date ( "i" );
...
function recup_date_tarif($date, $type_retour = 0, $type_terrain) {
	...
	if ($nbre_resultats > 0)
		$numero_jour_de_la_semaine = 7; // c'est comme dimanche
	else {
		list ( $annee, $mois, $jour ) = explode ( '-', $date );
	...
	$requete_recup_tarif = "SELECT * FROM `Plage_tarif` as pt, Tarif as t, Tarif_periode as tp, Tarif_periode_type_terrain as tptt " . " where pt.`id_tarif`=t.`id_tarif` and tp.id_tarif=t.id_tarif and tptt.id_periode=tp.id_periode and tptt.id_type_terrain=" . $type_terrain . " and " . $numero_jour_de_la_semaine . ">=pt.jour_debut_periode and " . $numero_jour_de_la_semaine . "<=jour_fin_periode ";
...
function tarif($date, $horaire_debut, $horaire_fin, $id_type_regroupement, $police, $type_terrain, $mode_resa, $tarif_special = -1, $type_terrain_texte = '') {
	...
	if (test_nuit_avant_ouverture ( $horaire_debut ) == 1)
		$date_deb = decaler_jour ( $date, 1 );
	else
		$date_deb = $date;
	
	if (test_nuit_avant_ouverture ( $horaire_fin ) == 1)
		$date_fin = decaler_jour ( $date, 1 );
	else
		$date_fin = $date;
	...
	for($i = 0; $i < $j; $i ++) {
		...
		if (test_nuit_avant_ouverture ( $tab_tarif [$i] ["heure_debut"] ) == 1)
			$date_Tab_deb = decaler_jour ( $date, 1 );
		else
			$date_Tab_deb = $date;
		
		if (test_nuit_avant_ouverture ( $tab_tarif [$i] ["heure_fin"] ) == 1)
			$date_Tab_fin = decaler_jour ( $date, 1 );
		else
			$date_Tab_fin = $date;
		...
		if (($diff_bornes_fin_fin >= 0) and ($diff_bornes_debut_debut > 0) and $diff_bornes_fin_debut < 0)
			$tarif = $tarif + (($tab_tarif [$i] ["montant_horaire"] / 60) * diff_dates_en_minutes ( $date_Tab_deb, $tab_tarif [$i] ["heure_debut"], $date_fin, $horaire_fin ));
		
		if (($diff_bornes_fin_fin < 0) and ($diff_bornes_debut_debut <= 0) and $diff_bornes_debut_fin > 0)
			$tarif = $tarif + (($tab_tarif [$i] ["montant_horaire"] / 60) * diff_dates_en_minutes ( $date_deb, $horaire_debut, $date_Tab_fin, $tab_tarif [$i] ["heure_fin"] ));
		...
		if ($mode_resa < 3 and ((! in_array ( $id_type_regroupement, array (
			...
			if (($diff_bornes_fin_fin >= 0) and ($diff_bornes_debut_debut > 0) and $diff_bornes_fin_debut < 0)
				$tarif_avec_remise = $tarif_avec_remise + (($tab_tarif [$i] ["montant_avec_remise"] / 60) * diff_dates_en_minutes ( $date_Tab_deb, $tab_tarif [$i] ["heure_debut"], $date_fin, $horaire_fin ));
			
			if (($diff_bornes_fin_fin < 0) and ($diff_bornes_debut_debut <= 0) and $diff_bornes_debut_fin > 0)
				$tarif_avec_remise = $tarif_avec_remise + (($tab_tarif [$i] ["montant_avec_remise"] / 60) * diff_dates_en_minutes ( $date_deb, $horaire_debut, $date_Tab_fin, $tab_tarif [$i] ["heure_fin"] ));
...
function ajout_event_cal_google($id_resa) {
	...
	$event->where = array (
			$cal->newWhere ( "T" . $recup_resa->id_terrain . " : " . $recup_resa->nom_long ) 
	);
...
function consult_cal_google($cal, $query, $date_Min, $horaire_Min, $date_Max, $horaire_Max) {
	...
	if (test_horaires_ouverture ( $horaire_Min, "" ) == 0)
		$horaire_Min = horaire_ouverture ();
	
	if (test_horaires_ouverture ( "", $horaire_Max ) == 0)
	...
	if (test_nuit_avant_ouverture ( $horaire_Min ) == 1)
		$date_Min = decaler_jour ( $date_Min, 1 );
	if (test_nuit_avant_ouverture ( $horaire_Max ) == 1)
		$date_Max = decaler_jour ( $date_Max, 1 );
...
function test_dispo($type_terrain, $date_saisie_Min, $horaire_Min, $horaire_Max, $terrain = "", $resa_modif = "") {
	...
	// test si en dehors des creneaux d'ouverture
	if (test_horaires_ouverture ( $horaire_Min, $horaire_Max ) == 0)
		return (0);
	...
	if (test_nuit_avant_ouverture ( $horaire_Min ) == 1) {
		...
		if (test_nuit_avant_ouverture ( $horaire_Max ) == 1)
			$date_Max = decaler_jour ( $date_saisie_Min, 1 );
		else
			$date_Max = $date_saisie_Min;
	}
	if ($heure_min < 9)
		$stricte = "=";
	else
	...
	foreach ( $resultat_test_dispo as $test_dispo ) {
		if ($terrain != "") {
			...
			$recup_infos_terrain [] = $test_dispo->id;
		// echo "[".$test_dispo->nom."]";
	}
...
function trouve_dispo($type_terrain, $date_saisie_Min, $horaire_Min, $horaire_Max, $num_resa, $rubiks_cube = false) {
	...
	if (is_array ( $tab_liste_terrains_dispo_sans_trous_demi_heure ))
		return ($tab_liste_terrains_dispo_sans_trous_demi_heure);
	else
		return (0);
}
...
function test_limite_nbre_resa_par_caution($date_debut_resa, $id_client) {
	$db = JFactory::getDBO ();
	$nbre_resas_meme_jour = 0;
	$limite_par_jour = 2;
	$limite_par_semaine = 2;
	$nbre_jours = 4;
	
	$requete_test_limite_jour = "(select count(id_resa) from Reservation where " . " `date_debut_resa`=\"" . $date_debut_resa . "\" " . " and id_client=" . $id_client . " and indic_annul<>1 and accompte_necessaire<>1 and cautionnable=1) ";
	...
	$requete_test_limite_semaine = "SELECT IFNULL((select count(id_resa) from Reservation where " . " ((TIMESTAMPDIFF(MINUTE, CAST(concat(`date_debut_resa`,\" \",`heure_debut_resa`) AS CHAR(22))," . " CAST(concat(\"" . $date_debut_resa . "\",\" \",\"00:00\") AS CHAR(22)))) >= " . (- 1 * $nbre_jours * 24 * 60) . ") " . " and ((TIMESTAMPDIFF(MINUTE, CAST(concat(`date_fin_resa`,\" \",`heure_fin_resa`) AS CHAR(22))," . " CAST(concat(\"" . $date_debut_resa . "\",\" \",\"00:00\") AS CHAR(22))))<= " . ($nbre_jours * 24 * 60) . ") " . " and id_client=" . $id_client . " and indic_annul<>1 and accompte_necessaire<>1 and cautionnable=1),0) ";
...
function recup_resa_si_2_validations($date_debut_resa, $heure_debut_resa, $heure_fin_resa, $terrain_choisit, $mode_resa) {
	...
	$requete_recup_resa_si_2_validations .= " and id_terrain=\"" . $terrain_choisit . "\" and indic_annul<>1 and id_mode_reservation=$mode_resa";
...
function recup_resa_sur_periode($date_debut_resa, $heure_debut_resa, $heure_fin_resa) {
	...
	$requete_recup_resa_sur_periode = "select * from Reservation where " . " date_debut_resa=\"" . $date_debut_resa . "\" and heure_debut_resa>=\"" . $heure_debut_resa . "\" " . " and heure_debut_resa<=\"" . $heure_fin_resa . "\" and indic_annul<>1 and bloquer_optimisation=0 ";
...
function ajout_resa($date_debut_resa, $date_fin_resa, $heure_debut_resa, $heure_fin_resa, $id_client, $terrain_choisit, $montant_total, $duree_resa, $cautionnable, $mode_reservation, $montant_sans_remise) {
	...
	$requete_ajout_resa .= "  cautionnable,accompte_necessaire,id_mode_reservation,montant_sans_remise,a_supprimer) VALUES (";
	if ($id_resa != "")
		$requete_ajout_resa .= $id_resa . ",";
...
function maj_match_ledg($id_match, $date_debut_resa, $heure_debut_resa, $terrain_choisit) {
	...
	$query = "UPDATE `vlxhj_bl_match` SET `m_date`=\"" . $date_debut_resa . "\"," . " `m_time`=\"" . Ajout_zero_si_absent ( $heure_debut_resa ) . "\",`m_location`=\"T" . $terrain_choisit . "\" WHERE `id`=" . $id_match;
	// echo $query;
...
function recup_infos_nbre_credit($id_moyen_paiement, $complement_date_requete, $origine_credit, $Type_credit = "", $signe = "") {
	...
	if ($Type_credit == "")
		$complement_requete_type_credit = "";
	else
		$complement_requete_type_credit = " and type_credit=" . $Type_credit;
	
	if ($signe == "")
		$complement_requete_signe = "";
	else
		$complement_requete_signe = " and credit" . $signe . "0 ";
...
function recup_infos_nbre_reglements($id_moyen_paiement, $complement_date_requete, $signe = "", $base = "") {
	...
	if ($signe == "")
		...
		$complement_requete_signe = " and montant_reglement" . $signe . "0 ";
...
function conditions_generales($fact_ou_dev) {
	if ($fact_ou_dev == "FACTURE")
		$texte = "<tr><td><br>Pas d'escompte en cas de paiement anticip&eacute;" . "<br>Int&eacute;r&ecirc;ts de retard pour paiement tardif 3 fois<br>" . "le taux de l'int&eacute;r&ecirc;t l&eacute;gal (Loi LME num 2008-776 du 4 ao&ucirc;t 2008).</td></tr>";
	else
		$texte = "<tr><td height=\"100%\"><b><u>Conditions g&eacute;n&eacute;rales :</u></b><br>Un acompte de 30&#37; doit &ecirc;tre d&eacute;pos&eacute; afin de confirmer" . " votre r&eacute;servation.<br>Le solde se fera le jour de l'&eacute;v&egrave;nement.<br>" . "En cas de d&eacute;sistement 48 heures avant, tout acompte ne pourra faire l'objet d'un remboursement.<br>" . "Les modes de paiement accept&eacute;s : CB, esp&eacute;ces et ch&egrave;que</td></tr>";
