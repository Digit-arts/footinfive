...
$requete_info_resa="Select format(r.montant_total,2) as le_montant_total, r.*,c.*, t.nom as le_terrain,t.id_type as type_terrain,t.id_cal_google,";
...
$requete_info_resa.=" ,(select ugm.group_id from #__user_usergroup_map as ugm where ugm.user_id=r.id_user) as gid_user FROM `Reservation` as r, Terrain as t , "
			." Client as c LEFT JOIN Type_Regroupement as tr on c.id_type_regroupement=tr.id ";
$requete_info_resa.="  where c.id_client=r.id_client and r.id_terrain=t.id ";
...
if (est_min_agent($user) and test_non_vide($_POST["terrains_dispo"])) {
	$id_resa_new=maj_resa($info_resa->id_resa,$info_resa->date_debut_resa,$info_resa->date_fin_resa, $info_resa->heure_debut_resa,
		      ...
		      $info_resa->duree_resa,$info_resa->cautionnable);
...
echo "<table class=\"zebra\" >";
		...
		if (recup_accompte_necessaire($id_client)==0) 
			echo "<img src=\"images/VIP-Empty-icon.png\" title=\"Client non-autoris&eacute; &agrave; faire des r&eacute;sa sans acompte\">";
...
echo "</table><hr><br>";
		...
		echo "<th>Montant<br>R&eacute;sa</th><th>Montant<br>Presta</th><th>";
		if (($info_resa->cautionnable==0) and ($total_versement=="" or $total_versement==0) and $info_resa->accompte_necessaire==0){
			echo "Acompte";
		...
		if  ($info_resa->indic_annul<>1){
			...
			if (diff_dates_en_minutes($info_resa->date_debut_resa,$info_resa->heure_debut_resa)<-2880 or (est_min_agent($user) and diff_dates_en_minutes($info_resa->date_debut_resa,$info_resa->heure_debut_resa)<120) or (est_min_manager($user)) ) {// plus de modif avant 48h
				...
				echo "&modif=1&num_resa=".$info_resa->id_resa."&type_terrain=".$info_resa->type_terrain."&date_debut_resa=".$info_resa->date_debut_resa."&heure_fin_resa=".$info_resa->heure_fin_resa."&heure_debut_resa=".$info_resa->heure_debut_resa."\" />";
		...
		if ($bloquer_optimisation==0)
			echo "<img src=\"images/bloquer_optimisation_0.png\" title=\"Resa déplaçable\">";
		else echo "<img src=\"images/bloquer_optimisation_1.png\" title=\"Resa non-déplaçable\">";
		...
		if ($info_resa->cautionnable==1 or  $info_resa->accompte_necessaire==1)
