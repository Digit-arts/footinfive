...
if (est_register ( $user ))
	...
	if (test_non_vide ( $_POST ["email"] ))
		$email = $_POST ["email"];
	else
		$email = $_GET ["email"];
	
	if (test_non_vide ( $_POST ["code_postal"] ))
	...
	if (test_non_vide ( $_POST ["tel"] ))
		$tel = $_POST ["tel"];
	else
		$tel = $_GET ["tel"];
	
	if (test_non_vide ( $_POST ["vip"] ))
		$vip = $_POST ["vip"];
	else
		$vip = $_GET ["vip"];
	
	if (test_non_vide ( $_POST ["joueur_champ"] ))
		$joueur_champ = $_POST ["joueur_champ"];
	else
		$joueur_champ = $_GET ["joueur_champ"];
	
	if (test_non_vide ( $_POST ["police"] ))
		$police = $_POST ["police"];
	else
		$police = $_GET ["police"];
	
	if (test_non_vide ( $_POST ["contremarque"] ))
		$contremarque = $_POST ["contremarque"];
	else
		$contremarque = $_GET ["contremarque"];
	
	if (test_non_vide ( $_POST ["nom_entite"] ))
		$nom_entite = $_POST ["nom_entite"];
	else
		$nom_entite = $_GET ["nom_entite"];
	
	if (test_non_vide ( $_POST ["Type_Regroupement"] ))
		$id_type_regroupement = $_POST ["Type_Regroupement"];
	else
		$id_type_regroupement = $_GET ["Type_Regroupement"];
	
	menu_acces_rapide ( "", "Liste des clients" );
	
	?>
<FORM id="formulaire" name="filtre" class="submission box"
	action="<?php echo $siteURL;?>/index.php?option=com_content&view=article&id=57&Itemid=247"
	method="post">
	<center>
		<table width="100%" border="0">
			<tr>
				<td><input name="id_client" type="text"
					value="<? echo $id_client;?>" size="10" placeholder="Num client"></td>
				<!--td><input name="Siret" type="text"  value="<? echo $Siret;?>" size="10"  placeholder="Siret"></td-->
				<td><input name="equipe" type="text" value="<? echo $equipe;?>"
					size="10" placeholder="Equipe"></td>
				<td><input name="nom" type="text" value="<? echo $nom;?>" size="10"
					placeholder="Nom"></td>
				<td><input name="prenom" type="text" value="<? echo $prenom;?>"
					size="10" placeholder="Prenom"></td>
				<td align="right"><INPUT type="checkbox" name="vip" value="1"
					<? if (test_non_vide($vip)) echo "checked"; ?>> <img
					src="images/VIP-icon.png" title="VIP" /></td>
			</tr>
			<tr>
				<td><input name="tel" type="text" value="<? echo $tel;?>"
					maxlenght="10" size="10" placeholder="Tel"></td>
				<td><input name="email" type="text" value="<? echo $email;?>"
					size="10" placeholder="Email"></td>
				<td><input name="code_postal" type="text"
					value="<? echo $code_postal;?>" size="2" maxlenght="5"
					placeholder="CP"></td>
				<td><input name="ville" type="text" value="<? echo $ville;?>"
					size="10" placeholder="Ville"></td>
				<td align="right"><INPUT type="checkbox" name="joueur_champ"
					value="1" <? if (test_non_vide($joueur_champ)) echo "checked"; ?>>
			...
			<tr>
				...
				<td><input name="nom_entite" type="text"
					value="<? echo $nom_entite;?>" size="10"
					placeholder="Soc/Assoc/Mairie"></td>

				<td colspan="3" align="right"><INPUT type="checkbox"
					name="contremarque" value="1"
					<? if (test_non_vide($contremarque)) echo "checked"; ?>> <img
					src="images/contremarque-icon.png" title="Contremarque" />&nbsp;&nbsp;
					<INPUT type="checkbox" name="police" value="1"
					<? if (test_non_vide($police)) echo "checked"; ?>> <img
					src="images/police-icon.png" title="Police" /></td>
			</tr>
			<tr>
				<td colspan=5 align=center><input name="valide" type="button"
					value="Filtrer" onclick="enregistrer()" /></td>
			</tr>
			<tr>
...
<FORM id="newsletter" name="newsletter" class="submission box"
	action="<?php echo $siteURL;?>/index.php?option=com_content&view=article&id=57&Itemid=247"
	method="post">
	<input name="update_newsletter" type="submit"
		value="Synchroniser newsletter" />
</form>
<br>
<?
	
	$requete_recup_client = "select c.*, tr.nom as nom_type,  " . "  u.email as courriel , v.code_postal, v.nom_maj_ville, c.adresse as adresse, date_naissance " . "From Type_Regroupement as tr , Client as c LEFT JOIN Ville as v ON c.code_insee=v.code_insee " . " LEFT JOIN #__users as u on u.id=c.id_user where 1  and tr.id=c.id_type_regroupement ";
	if (test_non_vide ( $email ))
		$requete_recup_client .= " and u.email like \"%" . $email . "%\"";
	if (test_non_vide ( $id_client ))
		$requete_recup_client .= " and c.id_client=" . $id_client;
	if (test_non_vide ( $nom ))
		$requete_recup_client .= " and c.nom like \"%" . $nom . "%\"";
	if (test_non_vide ( $prenom ))
		$requete_recup_client .= " and c.prenom like \"%" . $prenom . "%\"";
	if (test_non_vide ( $code_postal ))
		$requete_recup_client .= " and v.code_postal like \"%" . $code_postal . "%\"";
	if (test_non_vide ( $ville ))
		$requete_recup_client .= " and v.nom_maj_ville like \"%" . mb_strtoupper ( $ville ) . "%\"   ";
	if (test_non_vide ( $Siret ))
		$requete_recup_client .= " and c.Siret like \"%" . $Siret . "%\"   ";
	if (test_non_vide ( $equipe ))
		$requete_recup_client .= " and c.equipe like \"%" . $equipe . "%\"   ";
	if (test_non_vide ( $vip ))
		$requete_recup_client .= " and c.accompte_necessaire=1 ";
	if (test_non_vide ( $police ))
		$requete_recup_client .= " and c.police=1 ";
	if (test_non_vide ( $contremarque ))
		$requete_recup_client .= " and c.id_client in (SELECT id_client FROM `Contremarque` ) ";
	if (test_non_vide ( $nom_entite ))
		$requete_recup_client .= " and (c.nom_entite like \"%" . $nom_entite . "%\" or c.nom_service like \"%" . $nom_entite . "%\") ";
	if (test_non_vide ( $joueur_champ ))
		$requete_recup_client .= " and u.id in " . liste_users_ledg ();
	...
	if (test_non_vide ( $id_type_regroupement ))
		$requete_recup_client .= " and tr.id=" . $id_type_regroupement . "   ";
	...
	if (isset ( $_POST ['update_newsletter'] ) && $_POST ['update_newsletter'] == "Synchroniser newsletter") {
		$db->setQuery ( $requete_recup_client );
		$db->query ();
		$resultat_recup_client = $db->loadObjectList ();
		$ij = 0;
		if (file_exists ( 'libraries/ya2/DOCS_CLIENTS/BddClient_news.csv' )) {
			chmod ( 'libraries/ya2/DOCS_CLIENTS/BddClient_news.csv', 0777 );
			unlink ( 'libraries/ya2/DOCS_CLIENTS/BddClient_news.csv' );
		}
		
		$fp = fopen ( 'libraries/ya2/DOCS_CLIENTS/BddClient_news.csv', 'w+' );
		
		$fields = array('Email Address','First Name','Last Name','Type de joueur');
		fputcsv($fp, $fields,';');
		
		foreach ( $resultat_recup_client as $recup_client ) {
			$fields=array(
					$recup_client->courriel,
					$recup_client->prenom,
					$recup_client->nom,
					$recup_client->nom_type
			);
			fputcsv($fp, $fields,'|');
			//syncMailchimp ( $mailchimpAPIKey, $recup_client->nom, $recup_client->prenom, $recup_client->courriel, $recup_client->newsletter, $mailchimpURL, $recup_client->nom_type );			
		}
		
		fclose ( $fp );
	}
	
	$lien = "<a href=\"$siteURL/index.php?vip=" . $vip . "&joueur_champ=" . $joueur_champ . "&police=" . $police . "&contremarque=" . $contremarque . "&nom=" . $nom . "&nom_entite=" . $nom_entite . "&prenom=" . $prenom . "&email=" . $email . "&code_postal=" . $code_postal . "&ville=" . $ville . "&Siret=" . $Siret . "&equipe=" . $equipe . "&Type_Regroupement=" . $id_type_regroupement;
