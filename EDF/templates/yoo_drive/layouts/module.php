...
// init params
foreach (array('suffix', 'style', 'color', 'effect', 'badge', 'icon', 'dropdownwidth') as $var) {
...
switch ($style) {
	...
	case 'box':
		$template		= 'default-1';
		$style			= 'mod-'.$style;
		$style			.= isset($color) ? ' mod-box-'.$color : '';

		if (isset($effect)) {
			$suffix = trim(preg_replace("/effect-[a-z0-9]+/", "", $params['suffix']));
			$style	.= $effect=="on" ? ' mod-box-effect' : '';
		}
