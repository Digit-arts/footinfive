...
if (is_array($_FILES['doc'])){
	$uploaddir = '/var/www/vhosts/footinfive.com/httpdocs/FIF/libraries/ya2/DOCS_CLIENTS/';
...
echo "<table class=\"zebra\" >";
	...
	echo "</tr><tr>";
	?><form name="register" enctype="multipart/form-data" class="submission box" action="?id=69&id_client=<?echo $id_client;?>" method="post"  >
	<?
	    echo "<td>";
	...
	echo "<tr><th>Effectuer par</th><th>Date</th><th>Nom doc</th><th>Nom fichier</th><th>suppr</th></tr>";
		...
		echo "<tr>";
			...
			if  ($liste_doc->validation_doc==0) {
				...
				header("Location: ?id=69&id_client=".$id_client."");
			    }
			    else {
				if (est_min_agent($user))
				    echo "<a href=\"?id=69&reactiver_doc=".$liste_doc->id_doc."&id_client=".$id_client."\" />reactiver</a>";
			    }
			}
			else {
			    if (est_min_agent($user))	{
