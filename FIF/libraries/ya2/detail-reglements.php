...
if (test_non_vide($_POST["date_deb"])) $date_deb=$_POST["date_deb"];
	...
	$temp=$id_resa.$id_client.$nom.$prenom.$equipe.$Moyen_paiement.$indic_annul.$indic_valid.$remise;
...
$requete_liste_reg="select (select ugm.group_id from #__user_usergroup_map as ugm where ugm.user_id=reg1.id_user) as gid_user,";
$requete_liste_reg.=" (select name from #__users where id=reg1.id_user) as nom_user, ";
$requete_liste_reg.=" (select nom from Remise where id=reg1.id_remise) as la_remise, reg1.*,c.*,r.*, m.nom as moy_paie";
$requete_liste_reg.=" FROM  Client as c, Reservation as r, Reglement as reg1 left join Moyen_paiement as m on reg1.id_moyen_paiement=m.id ";
$requete_liste_reg.=" where reg1.id_client=c.id_client and r.id_resa=reg1.id_reservation ".$complement_req;
...
if (test_non_vide($id_resa))
	$requete_liste_reg.=" and reg1.id_reservation=".$id_resa;
...
if (test_non_vide($equipe)) $requete_liste_reg.=" and c.equipe like \"%".$equipe."%\"";
...
if (!$resultat_liste_reg) echo $prb;
	...
	echo "<th>Num résa</th>";
	echo "<th>Montant résa</th><th>Montant vers&eacute;</th><th>Etat</th><th>Moyen de paiement</th>";
	...
	foreach($resultat_liste_reg as $liste_reg){
		...
		if (est_min_agent($user)) {
			...
			if ($liste_reg->societe=="") {
				if ($liste_reg->equipe<>"")
					echo $liste_reg->equipe."<br>";
			}
			else echo $liste_reg->societe."<br>";
			echo $liste_reg->nom." ".$liste_reg->prenom."</a></td>";
		}
		echo "<td><a href=\"index.php/component/content/article?id=61&premiere=1&id_resa=".$liste_reg->id_reservation."\"/>";
		echo $liste_reg->id_reservation."</a> ";
		if  ($liste_reg->indic_annul==1) echo " <img src=\"images/Cancel-resa.png\" title=\"Réservation annulée\">";
		echo "</td><td>";
		echo str_replace(".", ",",$liste_reg->montant_total)." &#8364;</td><td>";
	...
	echo "<tr>";
		echo "<th>NBRE</th>";
		echo "<th>".$nbre_operations."</th><th colspan=2>Total vers.</th><th nowrap>".$total_versements_page." &#8364;</th><td colspan=5></th>";
	echo "</tr></table>";
}
