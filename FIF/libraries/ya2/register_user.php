...
if ($existe_erreur==0 and (isset($_GET["modif"]))  and test_non_vide($_POST["condition"])){
	...
	if (test_non_vide($_POST["id_client"])) {
		...
		$recup_old_client = $db->loadObject();
			...
			if (est_min_agent($user)) $requete_insert_old_client.=" ,societe, equipe";
			...
			if (est_min_agent($user)) $requete_insert_old_client.=" ,\"".$recup_old_client->societe."\",\"".$recup_old_client->equipe."\"";
			$requete_insert_old_client.=" )";
		...
		if (est_min_agent($user)) {
			...
			$requete_update_client.=" , `equipe`=\"".$_POST["equipe"]."\" , `nom_entite`=\"".$_POST["nom_entite"]."\", `nom_service`=\"".$_POST["nom_service"]."\" "
					.", `Raison_Sociale`=\"".$_POST["Raison_Sociale"]."\", `Siret`=\"".$_POST["Siret"]."\","
					." `Adresse_facturation`=\"".$_POST["Adresse_facturation"]."\", Ville_facturation=\"".$_POST["Ville_facturation"]."\", "
					." `Code_postal_facturation`=\"".$_POST["Code_postal_facturation"]."\", `Effectif`=\"".$_POST["Effectif"]."\"";
	
			if (test_non_vide($_POST["Type_Regroupement"]))
				$requete_update_client.=", id_type_regroupement=".$_POST["Type_Regroupement"]." ";
			else $requete_update_client.=", id_type_regroupement=0 ";
		}
	...
	if (!test_non_vide($_POST["id_client"])){
		...
		if (est_min_agent($user)) $requete_insert_client.=", societe,equipe, "
					."`id_type_regroupement`, `nom_entite`,nom_service, `Raison_Sociale`, `Siret`,"
					." `Adresse_facturation`, `Code_postal_facturation`,Ville_facturation, `Effectif`";
		...
		if (est_min_agent($user)) $requete_insert_client.=", \"".$_POST["societe"]."\",\"".$_POST["equipe"]."\","
					."\"".$_POST["Type_Regroupement"]."\",\"".$_POST["nom_entite"]."\",\"".$_POST["nom_service"]."\",\"".$_POST["Raison_Sociale"]."\","
					." \"".$_POST["Siret"]."\",\"".$_POST["Adresse_facturation"]."\","
					." \"".$_POST["Code_postal_facturation"]."\",\"".$_POST["Ville_facturation"]."\",\"".$_POST["Effectif"]."\"";
		$requete_insert_client.=" )";
	...
	if (test_non_vide($_GET["id_client"]) or (test_non_vide($user->id) and est_register($user) and !test_non_vide($_POST["prenom"]))){
		...
		if (test_non_vide($_POST["nom_service"])) $nom_service=$_POST["nom_service"];
		if (test_non_vide($_POST["Raison_Sociale"])) $Raison_Sociale=$_POST["Raison_Sociale"];
		if (test_non_vide($_POST["Siret"])) $Siret=$_POST["Siret"];
		if (test_non_vide($_POST["Effectif"])) $Effectif=$_POST["Effectif"];
		if (test_non_vide($_POST["Adresse"])) $Adresse=$_POST["Adresse"];
		if (test_non_vide($_POST["Adresse_facturation"])) $Adresse_facturation=$_POST["Adresse_facturation"];
		if (test_non_vide($_POST["cp"])) $cp=$_POST["cp"];
		if (test_non_vide($_POST["ville"])) $ville=$_POST["ville"];
		if (test_non_vide($_POST["Code_postal_facturation"])) $Code_postal_facturation=$_POST["Code_postal_facturation"];
		if (test_non_vide($_POST["Ville_facturation"])) $ville_facturation=$_POST["Ville_facturation"];
		...
		if (est_min_agent($user)) {
			...
			if (test_non_vide($_POST["societe"])) $societe=$_POST["societe"];
			if (test_non_vide($_POST["equipe"])) $equipe=$_POST["equipe"];
			if (test_non_vide($_POST["Type_Regroupement"])) $id_type_regroupement=$_POST["Type_Regroupement"];
		}
	}
?>
...
<?
	...
	if (test_non_vide($id_client)) {
		...
		<tr>
			...
			<?
			if (test_non_vide($info_ledg))
				echo "<td rowspan=17 valign=top width=\"100\"><img src=\"http://footinfive.com//LEDG/media/bearleague/"
					.photo_user($id_user)."\" width=\"100\" height=\"125\"></td>";
			?>
		</tr><?
	}
	?>
		<tr>
			...
			else echo recup_1_element("nom","Type_Regroupement","id",$id_type_regroupement);?>
			</td>
		</tr>
<?	
	if ($id_type_regroupement>0){?>
		<tr>
			...
			<td ><? if (isset($_GET["modif"]))
				echo "<input type=\"text\" name=\"nom_entite\" id=\"nom_entite\" size=\"40\" "
					." value=\"".$nom_entite."\"  maxlength=\"50\" />";
				else echo $nom_entite;
			?></td>
		</tr>
		...
		<tr>
			...
			<td ><? if (isset($_GET["modif"]))
				echo "<input type=\"text\" name=\"nom_service\" id=\"nom_service\" size=\"40\" "
					." value=\"".$nom_service."\"  maxlength=\"50\" />";
				else echo $nom_service;
			?></td>
		</tr>
		<?}
		...
		<tr>
			<th>SIRET </th>
			<td ><? if (isset($_GET["modif"]))
				echo "<input type=\"text\" name=\"Siret\" id=\"Siret\" size=\"40\" "
					." value=\"".$Siret."\"  maxlength=\"14\" />";
				else echo $Siret;
			?></td>
		</tr>
		<?}
		...
		<tr>
			...
			<td ><? if (isset($_GET["modif"]))
				echo "<input type=\"text\" name=\"Raison_Sociale\" id=\"Raison_Sociale\" size=\"40\" "
					." value=\"".$Raison_Sociale."\"  maxlength=\"30\" />";
				else echo $Raison_Sociale;
			?></td>
		</tr>
		...
		<tr>
			...
			<td ><? if (isset($_GET["modif"]))
					echo "<input type=\"number\" name=\"Effectif\" id=\"Effectif\" size=\"40\" "
						." value=\"".$Effectif."\"  maxlength=\"5\" />";
				else echo $Effectif;
			?></td>
		</tr>
		<?}?>
		<tr>
			...
			<td ><? if (isset($_GET["modif"]))
					echo "<input type=\"text\" name=\"Adresse_facturation\" id=\"Adresse_facturation\" size=\"40\" "
						." value=\"".$Adresse_facturation."\"  maxlength=\"50\" />";
				else echo $Adresse_facturation;
			?></td>
		</tr>
		<tr>
			...
			<td ><? 
				if (isset($_GET["modif"])){
					echo "<input type=\"text\" name=\"Code_postal_facturation\" id=\"Code_postal_facturation\" size=\"3\" "
						." value=\"".$Code_postal_facturation."\"  maxlength=\"5\" />";
					echo "<input type=\"text\" name=\"Ville_facturation\" id=\"Ville_facturation\" size=\"30\" "
						." value=\"".$ville_facturation."\"  maxlength=\"50\" />";
				}else echo $Code_postal_facturation." ".$ville_facturation;
			?></td>
		</tr>
...
?>
		<!--tr>
			...
			<?}
			else */echo "<input type=\"hidden\"  name=\"societe\" value=\"".$societe."\">".$societe." &nbsp;&nbsp;";?>
			</td>
		</tr-->
		<tr>
			<th>Equipe : </th>
			...
			<?
			if (isset($_GET["modif"])){?>
			<input type="text" name="equipe" id="equipe" size="40" value="<?php echo $equipe;?>" class="inputbox required" maxlength="50" />
			<?}
			else echo "<input type=\"hidden\"  name=\"equipe\" value=\"".$equipe."\">".$equipe;?>
			</td>
		</tr>		
		...
		<tr>
			...
			<?}
			else echo "<input type=\"hidden\"  name=\"prenom\" value=".$prenom.">".$prenom;?>
			</td>
		</tr>
		<?
		for ($i=1;$i<5;$i++){?>
